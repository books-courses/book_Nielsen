...
def increasing_open_rate(period_rng):
    return open_rate_with_factor_change(
        period_rng, np.random.uniform(1.01, 1.30)
    )


def decreasing_open_rate(period_rng):
    return open_rate_with_factor_change(
        period_rng, np.random.uniform(0.5, 0.99)
    )
...
## donation behavior
def produce_donations(
    period_rng, user_behavior, num_emails, use_id, user_join_year
):
    ...
    user_gives = (
        num_emails / (NUM_EMAILS_SENT_WEEKLY * len(period_rng)) * user_has
    )
            ...
            pd.DataFrame(
                {
                    ...
                    'amount': [
                        donation_amounts[
                            user_gives_idx + np.random.binomial(1, 0.3)
                        ]
                    ],
                    'timestamp': [
                        str(
                            period_rng[times[n]].start_time
...
for idx in range(yearJoined.shape[0]):
    ...
    donations = donations.append(
        produce_donations(
            user_rng, user_behaviors[idx], sum(info), idx, join_date.year
        )
    )
